...
class BinaryLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        binary_round = self.round(binary)
        
        last_one_position = binary_round.shape[1] - binary_round.flip(dims=[1]).argmax(dim=1) - 1

        last_one_position = torch.where(binary_round.sum(dim=1) == 0, torch.zeros_like(last_one_position), last_one_position)

        x = F.relu(self.be(binary_round))
        x = F.relu(self.ff3(x) + x)
        x = self.ff4(x)
        return x, last_one_position.detach(), binary[torch.arange(binary.size(0)) ,last_one_position]
    
class Embedding_Embedding_Model(nn.Module):
    def __init__(self, length):
        super(Embedding_Embedding_Model, self).__init__()
        self.Encoder = CoderLayer(2048)
        self.Binary = BinaryLayer(2048, length).to("cuda")
        self.Decoder = CoderLayer(2048)

    def forward(self, enc_src):
        enc_src = self.Encoder(enc_src)
        
        enc_src, last_one_position, value = self.Binary(enc_src)

        enc_src = self.Decoder(enc_src)

        return enc_src, last_one_position, value
...
class Combined_Model(nn.Module):
    ...
    def forward(self, input_ids, labels):
        ...
        enc_src, last_one_position, value = self.embedding_model(enc_src)
        output = self.transformer_model.forward_from_enc_src(enc_src, labels)
        return output, last_one_position, value
...
parser.add_argument('-alpha', type=float, help="一个小数参数", required=True)   
parser.add_argument('-alphatrain', type=float, help="一个小数参数", required=True)

# 解析命令行参数
args = parser.parse_args()


eemodel = Embedding_Embedding_Model(args.len).to(device)
eemodel.load_state_dict(torch.load(f"/root/autodl-tmp/embeddingmodel_Adjustment_{args.len}_{args.alpha}.pth", map_location=device))
...
wandb.init(project="encoder_decoder", entity="tqx_wandb", name=f"train_together_no_fixed_{args.len}_{args.alpha}_{args.alphatrain}_upd")
...
for epoch in range(10):
    ...
    for batch in bar:
        ...
        output, last_one_position, value = combined_model(input_ids, labels[:-1, :])
        ...
        loss2 = args.alphatrain * ((value * (last_one_position.float())).mean())
        loss = loss1 + loss2
        loss.backward()
        optimizer.step()
        wandb.log({"loss": loss.item(), "average_length": last_one_position.float().mean().item(), "loss1": loss1.item(), "loss2": loss2.item()})
        bar.set_description(f"loss: {loss.item()}, average_length: {last_one_position.float().mean().item()}, loss1: {loss1.item()}, loss2: {loss2.item()}")
        ...
        if cnt % 1024 == 1:
            ...
            print("output:", output)
            combined_model.train()
            torch.save(combined_model.state_dict(), f"/root/autodl-tmp/combined_model_no_fixlength_{args.len}_{args.alpha}_{args.alphatrain}.pth")