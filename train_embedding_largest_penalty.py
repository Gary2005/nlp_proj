...
parser.add_argument('-alpha', type=float, help="一个浮点数参数", required=True)
...
wandb.init(project="encoder_decoder", entity="tqx_wandb", name=f"embeddingmodel_multi_len_{args.len}_alpha_{args.alpha}")
...
for epoch in range(10):
    ...
    for batch in bar:
        ...
        optimizer.zero_grad()
        output, last_one_position, value = eemodel(enc_src)

        loss1 = mse_loss(output, enc_src)
        loss2 = alpha * ((value * (last_one_position.float())).mean())
        ...
        wandb.log({"loss1": loss1.item(), "loss1_log": math.log(loss1.item()), "loss2": loss2.item(), "loss2_log": math.log(loss2.item()), "average_length": last_one_position.float().mean()})
        bar.set_description(f"loss1: {loss1.item()}, loss1_log: {math.log(loss1.item())}, loss2: {loss2.item()}, loss2_log: {math.log(loss2.item())}, average_length: {last_one_position.float().mean()}")
        ...
        if cnt % 2048 == 1:
            ...
            torch.save(eemodel.state_dict(), f"/root/autodl-tmp/embeddingmodel_Adjustment_{args.len}_{args.alpha}.pth")
