...
class Transformer(nn.Module):
    ...
    def generate(self, src, max_len, start_symbol, end_symbol):
        """
        Perform inference (generate) given a source sequence.
        
        Args:
        src (torch.Tensor): Source sequence tensor.
        max_len (int): Maximum length of the target sequence to generate.
        start_symbol (int): The start symbol index for the target sequence.

        Returns:
        torch.Tensor: The generated target sequence.
        """
        self.eval()  # Set the model to evaluation mode
        
        src_mask = self.make_src_mask(src).permute(1, 0)
        src_positions = (torch.arange(0, src.shape[0]).unsqueeze(1).expand(src.shape[0], src.shape[1]).to(self.device))
        embed_src = self.src_word_embedding(src) + self.src_position_embedding(src_positions)
        enc_src = self.encoder(embed_src, src_key_padding_mask=src_mask)

        # (1, N) where N is the batch size
        outputs = (torch.ones(1, src.shape[1]).to(self.device) * start_symbol).type(torch.long)
        for _ in range(max_len):
            trg_positions = (torch.arange(0, outputs.shape[0]).unsqueeze(1).expand(outputs.shape[0], src.shape[1]).to(self.device))
            embed_trg = self.trg_word_embedding(outputs) + self.trg_position_embedding(trg_positions)

            trg_mask = self.generate_square_subsequent_mask(outputs.shape[0]).to(self.device)
            tgt_mask = self.make_src_mask(outputs).permute(1, 0)
            out = self.decoder(embed_trg, enc_src, tgt_mask=trg_mask, tgt_key_padding_mask=tgt_mask, memory_key_padding_mask=src_mask)
            out = self.fc_out(out)

            prob = out[-1, :, :].argmax(dim=1, keepdim=True)  # take the last output and find the highest probability
            if prob == end_symbol:  
                break
...
wandb.init(project="encoder_decoder", entity="tqx_wandb", name="origin_no_binary")
...
for epoch in range(10):
    ...
    for batch in bar:
        ...
        if cnt % 1024 == 1:
            ...
            print("output:", output)
            model.train()
            torch.save(model.state_dict(), "./truncate_transformer_origin.pth")
