...
class BinaryLayer(nn.Module):
    def __init__(self, mid_embedding_size):
        ...
        self.to_binary3 = nn.Linear(mid_embedding_size, mid_embedding_size)
...
eemodel = Embedding_Embedding_Model().to(device)
...
for epoch in range(10):
    ...
    for batch in bar:
        ...
        optimizer.zero_grad()
        output, mask_prob, mask_logits, mask_breakpoint = eemodel(enc_src)

        loss1 = mse_loss(output, enc_src)

        loss2 = torch.matmul(mask_prob, length_loss_weight).mean() * alpha

        loss3 = criterion_mask(mask_logits, mask_breakpoint) * beta

        loss = loss1 +loss2 + loss3
        loss.backward()
        optimizer.step()

        wandb.log({"loss1": loss1.item(), "loss1_log": math.log(loss1.item()), "average_length": mask_breakpoint.float().mean()})
        ...
        bar.set_description(f"loss1: {loss1.item()}, loss1_log: {math.log(loss1.item())}, average_length: {mask_breakpoint.float().mean()}")
        ...
        if cnt % 2048 == 1:
            ...
            torch.save(eemodel.state_dict(), "/root/autodl-tmp/embeddingmodel.pth")
