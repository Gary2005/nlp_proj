...
wandb.init(project="encoder_decoder", entity="tqx_wandb", name=f"embeddingmodel_len_{args.len}")

eemodel_pth = f"/root/autodl-tmp/embeddingmodel_{args.len}.pth"
eemodel.load_state_dict(torch.load(eemodel_pth))
eemodel = eemodel.to(device)
eemodel.train()
...
for epoch in range(10):
    ...
    for batch in bar:
        ...
        wandb.log({"loss1": loss1.item(), "loss1_log": math.log(loss1.item()),})
        bar.set_description(f"loss1: {loss1.item()}, loss1_log: {math.log(loss1.item())}")
        ...
        if cnt % 2048 == 1:
            ...
            torch.save(eemodel.state_dict(), f"/root/autodl-tmp/embeddingmodel_{args.len}.pth")
