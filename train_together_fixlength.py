...
eemodel.load_state_dict(torch.load(f"/root/autodl-tmp/embeddingmodel_{args.len}.pth", map_location=device))
...
wandb.init(project="encoder_decoder", entity="tqx_wandb", name=f"train_together_fixed_{args.len}_upd")
...
for epoch in range(10):
    ...
    for batch in bar:
        ...
        if cnt % 1024 == 1:
            ...
            print("output:", output)
            combined_model.train()
            torch.save(combined_model.state_dict(), f"/root/autodl-tmp/combined_model_{args.len}.pth")