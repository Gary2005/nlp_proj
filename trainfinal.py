...
class Transformer(nn.Module):
    def __init__(self, src_vocab_size, trg_vocab_size, src_pad_idx, trg_pad_idx, embed_size=256, num_layers=6, forward_expansion=4, heads=8, dropout=0.1, max_length=50, device="cuda", mid_embedding_size=2048):
        ...
        self.binary = BinaryLayer(mid_embedding_size)
...
wandb.init(project="encoder_decoder", entity="tqx_wandb", name="transformer")
...
for epoch in range(10):
    ...
    for batch in bar:
        ...
        if cnt % 2048 == 1:
            ...
            print("output:", output)
            model.train()
            torch.save(model.state_dict(), "./truncate_transformer2.pth")
